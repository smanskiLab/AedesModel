...
This code can be used to simulate SGD, FL, SSIMS, and SGI with either one release or periodic release
...
typeExp = 'SSIMS'              # This is used for Filename generation '[typeExp]experiment[#].pkl'
experiment = 90                      # This is the starting percentage of GE individuals

startingGMOgenotype = 'PPTTLL'      # SSIMS = 'PPTTLL', SGI = 'PPTTll', FL = 'ppttLL', SGD = 'GW'
...
addedGMO = 800                        # intermittently added GE agents
timeStepAdded = 5                   # number of time steps between intermittent addition.
timeSteps = 200                     # number of time steps to simulate (1 ts = 1.57 days); if >1000, adjust line 473
...
class AedesModel(Model):
    ...
    def __init__(self, N, width, height):
        ...
        self.L1L2MatrixList=[]
        self.zzz = 0
        for i in range(0,1000):                                              # change with timesteps
        ...
        self.datacollector = DataCollector(                                 #Stores agent data, some are commented out to reduce RAM requirements
...
filename = typeExp + '_experiment_' + str(experiment) +'.pkl'
            ...
            for j in range(gridHeight-(2*borderCells)):
                for k in range(gridWidth-(2*borderCells)):
                    for l in range(addedGMO):
                        ...
                        new.genotype = startingGMOgenotype
                        new.state = 'new'
                        new.lifeStage = np.random.randint(2,11+1)
